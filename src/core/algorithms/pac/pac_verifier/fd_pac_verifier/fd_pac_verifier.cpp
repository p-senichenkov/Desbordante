    ...
    for (std::size_t i = 0; i < num_rows; ++i) {
        for (std::size_t j = i + 1; j < num_rows; ++j) {
            ...
            for (std::size_t col_num = 0; col_num < first_lhs.size(); ++col_num) {
                auto lhs_dist = lhs_metrics_.Dist(first_lhs[col_num], second_lhs[col_num], col_num);
            ...
            if (add_to_gamma) {
                ...
                for (std::size_t col_num = 0; col_num < first_rhs.size(); ++col_num) {
                    max_rhs_dist = std::max(
                            max_rhs_dist,
                            rhs_metrics_.Dist(first_rhs[col_num], second_rhs[col_num], col_num));
                }
...
void FDPACVerifier::PreparePACTypeData() {
    ...
    lhs_metrics_.SetTypes(TypedRelation(), lhs_indices_);
    rhs_metrics_.SetTypes(TypedRelation(), rhs_indices_);

    PreparePairs();
}

void FDPACVerifier::ExecuteInternal() {
    LogCommonOptions();
    ...
    auto get_column_names = [&columns](config::IndicesType const& indices) {
        std::vector<std::string> column_names;
        column_names.reserve(indices.size());
        std::ranges::transform(
                indices, std::back_inserter(column_names),
                [&columns](config::IndexType idx) { return columns[idx]->GetName(); });
        return column_names;
    };
    pac_ = model::FDPAC{lhs_indices_, get_column_names(lhs_indices_),
                        rhs_indices_, get_column_names(rhs_indices_),
                        lhs_Deltas_,  std::move(epsilons),
                        delta};
...
FDPACVerifier::FDPACVerifier() {
    ...
    auto lhs_deltas_default = [this]() {
        return std::vector<double>(lhs_indices_.size(), kDefaultLhsDelta);
    };
    auto check_lhs_deltas_count = [this](std::vector<double> const& value) {
        if (value.size() != lhs_indices_.size()) {
            std::ostringstream msg;
            msg << "Expected " << lhs_indices_.size() << " Lhs deltas, got " << value.size();
    ...
    RegisterOption(kLhsRawIndicesOpt(&lhs_indices_, [this]() {
                   ...
                   }).SetConditionalOpts({{nullptr, {kLhsMetrics, kLhsDeltas}}}));
    ...
    RegisterOption(MetricsOption{kLhsMetrics, kDLhsMetrics}(
            lhs_metrics_.ValuePtr(), [this]() { return lhs_indices_.size(); }));
    RegisterOption(MetricsOption{kRhsMetrics, kDRhsMetrics}(
            rhs_metrics_.ValuePtr(), [this]() { return rhs_indices_.size(); }));
    RegisterOption(
