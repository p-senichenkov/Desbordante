...
/// of FD PAC verifier, i. e. as maximum distance between values
class TupleMetric {
private:
...
public:
    ...
    double LhsDist(pac::model::Tuple const& a, pac::model::Tuple const& b) const {
        return Distance(a, b, lhs_metrics_);
    }
    ...
    double RhsDistOnColumn(pac::model::Tuple const& a, pac::model::Tuple const& b,
                           std::size_t col_num) const {
        return rhs_metrics_[col_num].Dist(a[col_num], b[col_num]);
    }
};
}  // namespace algos::pac_verifier
