...
template <typename T>
boost::any VectorToAny(std::vector<T>&& vec) {
    return vec.empty() ? boost::any{} : std::move(vec);
}
}  // namespace
    ...
    FDPACVerifyingParams(CSVConfig const& csv_config, config::IndicesType&& lhs_indices,
        ...
        : rhs_arity(rhs_indices.size()),
          params({
                  ...
                  {kLhsDeltas, VectorToAny(std::move(lhs_deltas))},
...
INSTANTIATE_TEST_SUITE_P(
        ...
        testing::ValuesIn({
                ...
                // String data test
                FDPACVerifyingParams(kTestND, {1}, {2}, 1, 0.889, {0.2}, std::nullopt, std::nullopt,
                                     0.7),
                // Multi-column values
                FDPACVerifyingParams(kTestFDPAC, {0, 1}, {2, 3}, 0.07, 0.890, {1, 10}, std::nullopt,
                                     std::nullopt, 0.7),
                // PAC only on the right-hand side
                FDPACVerifyingParams(kMetricMovies, {1}, {2}, 6, 0.933, {1}, std::nullopt,
                                     std::nullopt, 0.8),
                // Pure FD
                FDPACVerifyingParams(kMetricMovies, {0, 1}, {2}, 0, 1),
                // Single cluster on the left-hand side ("anti-UCC PAC")
                FDPACVerifyingParams(kMetricCoords, {2}, {3}, 120.093, 0.787, {20}, std::nullopt,
                                     std::nullopt, 0.7),
                ...
                FDPACVerifyingParams(kMarineUrchins, {0}, {1}, 22, 0.993, {10}, std::nullopt,
                                     std::nullopt, 0.7, {nullptr}, {nullptr}),
                //   b. [default] -> [custom]
                FDPACVerifyingParams(kMarineUrchins, {0}, {2}, 2, 0.686, {10}, std::nullopt,
                                     std::nullopt, 0.6, {}, {kStaticStringMetric}),
                ...
                // Only max epsilon
                FDPACVerifyingParams(kMetricCoords, {2}, {3}, 0.007, 0.66, {0.2}, std::nullopt,
                                     0.025),
                // Only min epsilon
                FDPACVerifyingParams(kMetricMovies, {1}, {2}, 6, 0.933, {0.2}, 5.5, std::nullopt),
                // Only min delta
                FDPACVerifyingParams(kMetricMovies, {1}, {2}, 6, 0.933, {0.2}, std::nullopt,
                                     std::nullopt, 0.7),
                // Min epsilon and min delta
                FDPACVerifyingParams(kMetricMovies, {1}, {2}, 6, 0.933, {0.2}, 4.9, std::nullopt,
                                     0.73),
                // Max epsilon and min delta
                FDPACVerifyingParams(kMetricMovies, {1}, {2}, 1, 0.6, {0.2}, std::nullopt, 1.9,
                                     0.5),
                // All bounds
...
struct FDPACHighlightParams {
    ...
    FDPACHighlightParams(CSVConfig const& csv_config, config::IndicesType&& lhs_indices,
                         ...
                         double eps_1 = 0, double eps_2 = -1, std::vector<double> lhs_deltas = {})
        : params({
                  {kCsvConfig, csv_config},
                  {kLhsIndices, std::move(lhs_indices)},
                  {kRhsIndices, std::move(rhs_indices)},
